...
<body>
    ...
    <table class="ref-table">
        <tr>
            ...
            <td class="reference">
                ...
                Date: <strong>{{ $generatedAt->format(\App\Support\ReportFormatter::DATE_FORMAT) }}</strong><br>
                Effective: <strong>{{ \App\Support\ReportFormatter::date($letter->effective_date) }}</strong>
            </td>
        </tr>
    </table>
    ...
    <p>
        ...
        <strong>{{ \App\Support\ReportFormatter::date($application->sitting?->sitting_date) }}</strong>, your promotion has been
        <strong>{{ str_replace('_', ' ', $application->decision ?? 'approved') }}</strong>.
    </p>

    <p>
        Your promotion takes effect from <strong>{{ \App\Support\ReportFormatter::date($letter->effective_date) }}</strong>. Your records
    ...
    <div class="footer">
        Generated by eHRMIS on {{ $generatedAt->format(\App\Support\ReportFormatter::DATETIME_FORMAT) }}. Verify against the official staff record before payroll processing.
    </div>
</body>
</html>
